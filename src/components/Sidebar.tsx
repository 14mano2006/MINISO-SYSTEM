...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-72 shrink-0 h-screen bg-[#ffffff] border-r border-[#efeeec] shadow-[0_1px_8px_rgba(0,0,0,0.04)] z-30 flex flex-col justify-between overflow-y-auto select-none">
      <div className="p-4 flex flex-col gap-4">
        {/* Logo and Brand header */}
        <div className="flex items-center justify-between pb-2 border-b border-[#f4f3f1]">
          <div className="flex items-center gap-2.5">
            <div className="h-9 w-9 rounded-xl bg-[#bb0012] flex items-center justify-center text-white font-extrabold text-xl shadow-sm tracking-tighter">
              M
            </div>
          ...
          <span className="bg-[#e9e8e6] text-[#5e5e65] text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-semibold">
            Java Prototype
          </span>
        </div>

        {/* View mode toggle (OS vs Streamlit) */}
        <div className="bg-[#f4f3f1] p-1 rounded-lg flex items-center gap-1 text-xs font-semibold">
          <button
            type="button"
            onClick={() => setIsStreamlitMode(false)}
            className={`flex-1 py-1.5 px-2 rounded-md transition-all flex items-center justify-center gap-1.5 ${
              !isStreamlitMode
                ? 'bg-white text-[#bb0012] shadow-sm font-bold'
          ...
          <button
            type="button"
            onClick={() => setIsStreamlitMode(true)}
            className={`flex-1 py-1.5 px-2 rounded-md transition-all flex items-center justify-center gap-1.5 ${
              isStreamlitMode
                ? 'bg-[#ff4b4b] text-white shadow-sm font-bold'
                : 'text-[#5e5e65] hover:text-[#1a1c1b]'
            }`}
          >
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (isStreamlitMode) setIsStreamlitMode(false);
                }}
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm transition-all text-left w-full ${
                  isActive
                    ? 'bg-[#bb0012] text-white font-semibold shadow-[0_2px_6px_rgba(187,0,18,0.25)]'
                    : 'text-[#5e5e65] hover:bg-[#f4f3f1] hover:text-[#1a1c1b] font-medium'
                }`}
              >
      ...
      <div className="p-4 border-t border-[#f4f3f1]">
        <div className="bg-[#f4f3f1] p-3 rounded-xl flex flex-col gap-1.5">
          ...
          <button
            ...
            className="mt-1 w-full bg-white hover:bg-[#e9e8e6] text-[#1a1c1b] text-xs font-semibold py-1.5 px-2 rounded-lg transition-colors flex items-center justify-center gap-1.5 shadow-sm"
          >
